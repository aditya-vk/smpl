...
#define DEFER(fun)                                                             \
  auto MAKE_LINE_IDENT(tmp_call_on_destruct_)                                  \
      = ::smpl::detail::MakeCallOnDestruct([&]() { fun; })

///////////////////////////////
// RobotModel Implementation //
///////////////////////////////

struct RobotModel : public virtual smpl::RobotModel,
                    public smpl::ForwardKinematicsInterface
{
  ...
  bool checkJointLimits(
      const smpl::RobotState& state, bool verbose = false) override;
...
bool CollisionChecker::isStateValid(const smpl::RobotState& state, bool verbose)
{
...
bool CollisionChecker::isStateToStateValid(
    const smpl::RobotState& start, const smpl::RobotState& finish, bool verbose)
{
...
auto CollisionChecker::getExtension(size_t class_code) -> smpl::Extension*
{
...
public:
  ...
  void setProblemDefinition(
      OMPLPlanner* planner, const ompl::base::ProblemDefinitionPtr& pdef);

  auto solve(
      OMPLPlanner* planner, const ompl::base::PlannerTerminationCondition& ptc)
    ...
    OccupancyGrid* grid)
{
  ...
  for (auto& location : locations)
  {
    ...
    SMPL_DEBUG(
        "  dimension = %u", location.stateLocation.space->getDimension());
  }
  ...
  this->checker.validator
      = planner->getSpaceInformation()->getMotionValidator().get();
  ...
  if (!this->space.init(
          &this->model, &this->checker, resolutions, &this->actions))
  {
  ...
  {
    auto set = [&](bool val) {
      this->actions.useAmp(smpl::MotionPrimitive::SNAP_TO_XYZ, val);
    };
    auto get = [&]() {
      return this->actions.useAmp(smpl::MotionPrimitive::SNAP_TO_XYZ);
    };
  ...
  {
    auto set = [&](bool val) {
      this->actions.useAmp(smpl::MotionPrimitive::SNAP_TO_RPY, val);
    };
    auto get = [&]() {
      return this->actions.useAmp(smpl::MotionPrimitive::SNAP_TO_RPY);
    };
  ...
  {
    auto set = [&](bool val) {
      this->actions.useAmp(smpl::MotionPrimitive::SNAP_TO_XYZ_RPY, val);
    };
    auto get = [&]() {
      return this->actions.useAmp(smpl::MotionPrimitive::SNAP_TO_XYZ_RPY);
    };
  ...
  {
    auto set = [&](bool val) {
      this->actions.useAmp(smpl::MotionPrimitive::SHORT_DISTANCE, val);
    };
    auto get = [&]() {
      return this->actions.useAmp(smpl::MotionPrimitive::SHORT_DISTANCE);
    };
  ...
  {
    auto set = [&](double val) {
      this->actions.ampThresh(smpl::MotionPrimitive::SNAP_TO_XYZ, val);
    };
    auto get = [&]() {
      return this->actions.ampThresh(smpl::MotionPrimitive::SNAP_TO_XYZ);
    };
  ...
  {
    auto set = [&](double val) {
      this->actions.ampThresh(smpl::MotionPrimitive::SNAP_TO_RPY, val);
    };
    auto get = [&]() {
      return this->actions.ampThresh(smpl::MotionPrimitive::SNAP_TO_RPY);
    };
  ...
  {
    auto set = [&](double val) {
      this->actions.ampThresh(smpl::MotionPrimitive::SNAP_TO_XYZ_RPY, val);
    };
    auto get = [&]() {
      return this->actions.ampThresh(smpl::MotionPrimitive::SNAP_TO_XYZ_RPY);
    };
  ...
  {
    auto set = [&](double val) {
      this->actions.ampThresh(smpl::MotionPrimitive::SHORT_DISTANCE, val);
    };
    auto get = [&]() {
      return this->actions.ampThresh(smpl::MotionPrimitive::SHORT_DISTANCE);
    };
    planner->params().declareParam<double>(
        "short_dist_mprims_thresh", set, get);
  }
...

  {
    ...
    {
      {
        auto set = [bfs_heuristic](double val) {
          bfs_heuristic->setInflationRadius(val);
        };
        auto get
            = [bfs_heuristic]() { return bfs_heuristic->inflationRadius(); };
        planner->params().declareParam<double>(
            "bfs_inflation_radius", set, get);
      }

      {
        auto set
            = [bfs_heuristic](int val) { bfs_heuristic->setCostPerCell(val); };
    ...
    -> ompl::base::PlannerStatus
{
...

  {
    ...
    SMPL_DEBUG(
        "Received goal of type %s", to_cstring(abstract_goal->getType()));
...
PoseGoal::PoseGoal(
    const ompl::base::SpaceInformationPtr& si, const Eigen::Affine3d& pose)
  : ompl::base::Goal(si), pose(pose)
{
}
...
OMPLPlanner::OMPLPlanner(
  ...
  , m_impl(
        make_unique<smpl::detail::PlannerImpl>(
...
void OMPLPlanner::setProblemDefinition(
...
auto MakeStateOMPL(
    const ompl::base::StateSpace* space, const smpl::RobotState& state)
    -> ompl::base::State*
{
...
auto MakeStateOMPL(
    const ompl::base::StateSpacePtr& space, const smpl::RobotState& state)
